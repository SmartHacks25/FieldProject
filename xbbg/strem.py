...
def run_with_defaults(path_to_tickers):
    ...
    run_fetch_data(path_to_tickers, frequency, start_date, new_data)
    log_successful_run()

# Function to fetch data
def run_fetch_data(path_to_tickers, frequency, start_date, new_data):
...
def create_input_window(path_to_tickers):
    ...
    if add_del in ['Delete']:
        ...
        frequency = st.selectbox("Select frequency", ['D', 'W', 'M'], index=2)
...
"""
            if errors:
                ...
                empty_data_error = run_fetch_data(path_to_tickers, frequency, start_date, new_data)
